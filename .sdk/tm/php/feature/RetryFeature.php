...
// CurrencyExchange SDK retry feature
...
class CurrencyExchangeRetryFeature extends CurrencyExchangeBaseFeature
{
    ...
    public function init(CurrencyExchangeContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CurrencyExchangeContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CurrencyExchangeContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CurrencyExchangeContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
