...
// CurrencyExchange SDK feature test
//
...
class FtCtrl extends CurrencyExchangeControl
{
...
class FtClient
{
    ...
    private CurrencyExchangeUtility $utility;

    public function __construct(CurrencyExchangeUtility $utility, array $options)
    {
    ...
    public function get_utility(): CurrencyExchangeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CurrencyExchangeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CurrencyExchangeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CurrencyExchangeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CurrencyExchangeConfig::make_feature($name);
    ...
    private static function buildUrl(CurrencyExchangeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CurrencyExchangeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CurrencyExchangeResponse($fetched);
        }
    ...
    private function fail(CurrencyExchangeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CurrencyExchangeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
