...
// CurrencyExchange SDK pipeline test
//
...
{
    private static function utility(): CurrencyExchangeUtility
    {
        return new CurrencyExchangeUtility();
    }
    ...
    private static function ctx(array $over = []): CurrencyExchangeContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CurrencyExchangeContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CurrencyExchangeOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CurrencyExchangeError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CurrencyExchangeOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CurrencyExchangeMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CurrencyExchangeOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CurrencyExchangeMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CurrencyExchangeOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CurrencyExchangeMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CurrencyExchangeSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CurrencyExchangeMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            ...
            'result' => new CurrencyExchangeResult([]),
        ]);
        [, $err] = CurrencyExchangeMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new CurrencyExchangeSpec([]),
            'response' => null,
            'result' => new CurrencyExchangeResult([]),
        ]);
        [, $err] = CurrencyExchangeMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CurrencyExchangeSpec(['step' => 's']),
            'response' => new CurrencyExchangeResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CurrencyExchangeResult([]),
        ]);
        [, $err] = CurrencyExchangeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CurrencyExchangeSpec(['step' => 's']),
            'response' => new CurrencyExchangeResponse(self::resp(200, ['v' => 1])),
            'result' => new CurrencyExchangeResult([]),
        ]);
        [, $err] = CurrencyExchangeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CurrencyExchangeSpec(['step' => 's']),
            'response' => new CurrencyExchangeResponse(self::resp(200, ['v' => 2])),
            'result' => new CurrencyExchangeResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CurrencyExchangeMakeResponse::call($ctx);
    ...
    {
        $preset = new CurrencyExchangeResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CurrencyExchangeSpec([]),
            'response' => new CurrencyExchangeResponse([]),
            'result' => new CurrencyExchangeResult([]),
        ...
        [$got, $err] = CurrencyExchangeMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CurrencyExchangeResult([])]);
        [, $err] = CurrencyExchangeMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CurrencyExchangeSpec([]), 'result' => null]);
        [, $err] = CurrencyExchangeMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CurrencyExchangeSpec(['step' => 's']),
            'result' => new CurrencyExchangeResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CurrencyExchangeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CurrencyExchangeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CurrencyExchangeSpec(['step' => 's']),
            'result' => new CurrencyExchangeResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CurrencyExchangeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CurrencyExchangeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CurrencyExchangeResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CurrencyExchangeSpec([]),
            'result' => new CurrencyExchangeResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CurrencyExchangeMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CurrencyExchangeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new CurrencyExchangeError('boom', 'boom');
        $utility->fetcher = function (CurrencyExchangeContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CurrencyExchangeSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CurrencyExchangeMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CurrencyExchangeContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CurrencyExchangeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CurrencyExchangeResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        CurrencyExchangeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CurrencyExchangeContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CurrencyExchangeSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CurrencyExchangeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CurrencyExchangeResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CurrencyExchangeSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CurrencyExchangeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CurrencyExchangeMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CurrencyExchangeMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CurrencyExchangeResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CurrencyExchangeDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CurrencyExchangeResult(['ok' => false])]);
        $this->expectException(CurrencyExchangeError::class);
        CurrencyExchangeDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CurrencyExchangeResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CurrencyExchangeMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        CurrencyExchangeMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CurrencyExchangeBaseFeature();
        $b = new CurrencyExchangeBaseFeature();
        CurrencyExchangeFeatureAdd::call($ctx, $a);
        CurrencyExchangeFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CurrencyExchangeBaseFeature
    {
        $f = new CurrencyExchangeBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        CurrencyExchangeFeatureAdd::call($ctx, self::named_feature('a'));
        CurrencyExchangeFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        CurrencyExchangeFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        CurrencyExchangeFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        CurrencyExchangeFeatureAdd::call($ctx, $replace);
        ...
        CurrencyExchangeFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?CurrencyExchangeSpec $spec): CurrencyExchangeContext
    {
    ...
    private static function auth_bags(): CurrencyExchangeSpec
    {
        return new CurrencyExchangeSpec(['headers' => [], 'query' => []]);
    }
    ...
    private static function auth_probe(array $options): ?array
    {
        ...
        CurrencyExchangePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CurrencyExchangePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new CurrencyExchangeResponse(['headers' => null]),
            'result' => new CurrencyExchangeResult([]),
        ]);
        CurrencyExchangeResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        CurrencyExchangeResultBody::call($ctx);
