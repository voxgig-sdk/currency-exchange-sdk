...
boru-driven command-line client **and** interactive REPL for the CurrencyExchange
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/currency-exchange-cli)
make build

# 2. See usage (words, entities, env vars)
./currency-exchange-cli --help

# 3. Provide credentials once, via the environment
export CURRENCY_EXCHANGE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./currency-exchange-cli load 1 convert            # {id:1} shorthand
./currency-exchange-cli load '{id:1}' convert       # explicit match map

# 5. Override the API base URL for a single call
CURRENCY_EXCHANGE_BASE=https://api.example.com ./currency-exchange-cli load 1 convert

# 6. No arguments -> interactive REPL
./currency-exchange-cli
currency-exchange> load 1 convert
currency-exchange> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/currency-exchange-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/currency-exchange-cli load 1 convert
   ```
...
```sh
./currency-exchange-cli load 1 convert          # scalar shorthand for {id:1}
./currency-exchange-cli load '{id:1}' convert     # explicit match map
```
...
./currency-exchange-cli load 1 convert
```
...
Run with no arguments to open a REPL (prompt `currency-exchange>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/currency-exchange-cli`. |
...
(`./currency-exchange-cli <expr>`) and interactively (the REPL), and expressions compose the
