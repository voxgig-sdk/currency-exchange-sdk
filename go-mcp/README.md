...
[MCP](https://modelcontextprotocol.io) server exposing the CurrencyExchange SDK as
two agent tools — `currency-exchange_list` and `currency-exchange_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/currency-exchange-mcp)
make build

# 2. Provide credentials via the environment
export CURRENCY_EXCHANGE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user currency-exchange \
  -- /absolute/path/to/currency-exchange-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./currency-exchange-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// currency-exchange_list: first page of records
{ "entity": "convert" }
{ "entity": "convert", "query": { } }

// currency-exchange_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/currency-exchange-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user currency-exchange \
     -- "$PWD"/dist/*/currency-exchange-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `currency-exchange_list` and `currency-exchange_load` tools now appear
   in new sessions. Ask the agent to *"list convert using currency-exchange"*
   and it calls `currency-exchange_list` with `{"entity":"convert"}`.
...
```sh
export CURRENCY_EXCHANGE_APIKEY=sk_live_xxx            # API key
export CURRENCY_EXCHANGE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `currency-exchange_list` tool
...
### Call the `currency-exchange_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `currency-exchange_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `currency-exchange_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CURRENCY_EXCHANGE_APIKEY` | API key sent with every request. |
| `CURRENCY_EXCHANGE_BASE` | Optional override of the API base URL. |
...
```sh
./currency-exchange-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"currency-exchange_load","arguments":{"entity":"convert","query":{"id":1}}}}'
```
