...
type Args struct {
	Entity string         `json:"entity" jsonschema:"convert | rate"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.CurrencyExchangeSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "currency-exchange_list",
		Description: "List records from CurrencyExchange. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "currency-exchange_load",
		Description: "Load a single record from CurrencyExchange. " +
...
func runOp(client *sdk.CurrencyExchangeSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.CurrencyExchangeSDK, name string) (sdk.CurrencyExchangeEntity, error) {
