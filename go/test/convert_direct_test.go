...
func TestConvertDirect(t *testing.T) {
	t.Run("direct-load-convert", func(t *testing.T) {
		setup := convertDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-convert", _mode); _shouldSkip {
			if _reason == "" {
...
func convertDirectSetup(mockres any) *convertDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"CURRENCYEXCHANGE_TEST_CONVERT_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CURRENCYEXCHANGE_TEST_CONVERT_ENTID"]; ok {
