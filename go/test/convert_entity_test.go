...
func TestConvertEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CURRENCY_EXCHANGE_TEST_CONVERT_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		convertRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.convert")))
...
func convertBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CURRENCY_EXCHANGE_TEST_CONVERT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CURRENCY_EXCHANGE_TEST_CONVERT_ENTID"])
