...
import (
	...
	omni "github.com/voxgig-sdk/currency-exchange-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CurrencyExchangeError); is {
				out := map[string]any{
					"name":    "CurrencyExchangeError",
...
func omniCtx(arg any, client *sdk.CurrencyExchangeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CurrencyExchangeSDK); is {
			client = live
