...
func TestRateDirect(t *testing.T) {
	t.Run("direct-load-rate", func(t *testing.T) {
		setup := rateDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-rate", _mode); _shouldSkip {
			if _reason == "" {
...
type rateDirectSetupResult struct {
...
func rateDirectSetup(mockres any) *rateDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		return &rateDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &rateDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
