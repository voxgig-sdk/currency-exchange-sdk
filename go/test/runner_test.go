...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("CURRENCY_EXCHANGE_TEST_LIVE") == "TRUE" ||
		os.Getenv("CURRENCY_EXCHANGE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("CURRENCY_EXCHANGE_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.CurrencyExchangeSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.CurrencyExchangeSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.CurrencyExchangeError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.CurrencyExchangeSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.CurrencyExchangeError{Msg: msg, Code: code}
}
