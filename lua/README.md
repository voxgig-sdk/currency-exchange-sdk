...
The Lua SDK for the CurrencyExchange API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Convert(nil):load({ id = "example_id" }, nil)
...
local result, err = client:CurrencyExchange(nil):load(
  { id = "test01" }, nil
)
