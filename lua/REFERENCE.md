# CurrencyExchange Lua SDK Reference

Complete API reference for the CurrencyExchange Lua SDK.
...
```lua
local result, err = client:Convert():load({ from = "from", key = "key", to = "to" })
```
...
```lua
local result, err = client:Rate():load({ base = "base", key = "key" })
```
