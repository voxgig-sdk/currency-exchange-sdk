...
require("currency-exchange_types")
...
local CurrencyExchangeSDK = {}
CurrencyExchangeSDK.__index = CurrencyExchangeSDK
...
CurrencyExchangeSDK._make_feature = _make_feature


function CurrencyExchangeSDK.new(options)
  local self = setmetatable({}, CurrencyExchangeSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function CurrencyExchangeSDK:options_map()
...
function CurrencyExchangeSDK:get_utility()
  return Utility.copy(self._utility)
end


function CurrencyExchangeSDK:get_root_ctx()
  return self._rootctx
end


function CurrencyExchangeSDK:prepare(fetchargs)
...
function CurrencyExchangeSDK:direct(fetchargs)
...
function CurrencyExchangeSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function CurrencyExchangeSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "CurrencyExchangeSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function CurrencyExchangeSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function CurrencyExchangeSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "CurrencyExchangeSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function CurrencyExchangeSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = CurrencyExchangeSDK.new(sdkopts)
  sdk.mode = "test"
