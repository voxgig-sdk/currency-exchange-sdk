...
local RateEntity = {}
RateEntity.__index = RateEntity


function RateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RateEntity)
  self._name = "rate"
  self._client = client
...
function RateEntity:get_name()
  return self._name
end


function RateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RateEntity.new(self._client, opts)
end
...
function RateEntity:mark_deleted()
  self._deleted = true
end


function RateEntity:deleted()
  return true == self._deleted
end


function RateEntity:data_set(args)
  if args ~= nil then
...
function RateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RateEntity:match_set(args)
  if args ~= nil then
...
function RateEntity:stream(action, args, callopts)
...
function RateEntity:_run_op(ctx, post_done)
  local utility = self._utility
