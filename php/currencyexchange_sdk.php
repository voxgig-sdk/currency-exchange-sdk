...
class CurrencyExchangeSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = CurrencyExchangeHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = CurrencyExchangeHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!CurrencyExchangeFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, CurrencyExchangeFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return CurrencyExchangeUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = CurrencyExchangeHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = CurrencyExchangeHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = CurrencyExchangeHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new CurrencyExchangeError($op . "_allow",
                "CurrencyExchangeSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = CurrencyExchangeHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new CurrencyExchangeError("graphql_error",
                "CurrencyExchangeSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Rate($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_rate === null) {
                $this->_rate = new RateEntity($this, null);
            }
            return $this->_rate;
        }
        return new RateEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new CurrencyExchangeSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
