...
// CurrencyExchange SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class CurrencyExchangeTestFeature extends CurrencyExchangeBaseFeature
{
    ...
    public function init(CurrencyExchangeContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (CurrencyExchangeContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = CurrencyExchangeParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (CurrencyExchangeContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(CurrencyExchangeContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = CurrencyExchangeParam::call($ctx, $k);
