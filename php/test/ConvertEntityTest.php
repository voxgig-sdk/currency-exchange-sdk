...
class ConvertEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CURRENCY_EXCHANGE_TEST_CONVERT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $convert_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.convert")));
        $convert_ref01_data = null;
        if (count($convert_ref01_data_raw) > 0) {
            $convert_ref01_data = Helpers::to_map($convert_ref01_data_raw[0][1]);
        }
...
function convert_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("CURRENCY_EXCHANGE_TEST_CONVERT_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["CURRENCY_EXCHANGE_TEST_CONVERT_ENTID"]);
