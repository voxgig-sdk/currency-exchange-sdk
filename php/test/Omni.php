...
//    only - a typed CurrencyExchangeContext there would make every ctx
...
final class CurrencyExchangeOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new CurrencyExchangeContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new CurrencyExchangeSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new CurrencyExchangeResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new CurrencyExchangeError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new CurrencyExchangeResponse($clean['response']);
    ...
    public static function err_from_map($m): ?CurrencyExchangeError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new CurrencyExchangeError(is_string($code) ? $code : '', $msg);
    }
