...
class CurrencyExchangeMakeRequest
{
    public static function call(CurrencyExchangeContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new CurrencyExchangeResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new CurrencyExchangeResponse($fetched);
        } else {
