# CurrencyExchange SDK test runner
...
class CurrencyExchangeTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CurrencyExchangeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CurrencyExchangeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CurrencyExchangeTestRunner.getenv("CURRENCY_EXCHANGE_TEST_LIVE")
        override = CurrencyExchangeTestRunner.getenv("CURRENCY_EXCHANGE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CurrencyExchangeTestRunner.getenv(key)
        ...
        explain = CurrencyExchangeTestRunner.getenv("CURRENCY_EXCHANGE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["CURRENCY_EXCHANGE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if CurrencyExchangeTestRunner._test_control is not None:
            return CurrencyExchangeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CurrencyExchangeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CurrencyExchangeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CurrencyExchangeTestRunner.load_test_control()
...
def load_env_local():
    CurrencyExchangeTestRunner.load_env_local()


def env_override(m):
    return CurrencyExchangeTestRunner.env_override(m)


def entity_data(v):
    return CurrencyExchangeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CurrencyExchangeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CurrencyExchangeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CurrencyExchangeTestRunner.load_test_control()


def live_client_options():
    return CurrencyExchangeTestRunner.live_client_options()


def live_delay_ms():
    return CurrencyExchangeTestRunner.live_delay_ms()
