...
class TestConvertDirect:
    ...
    def test_should_direct_load_convert(self):
        setup = _convert_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-convert", "live" if setup["live"] else "unit")
        if _skip:
...
def _convert_direct_setup(mockres):
