...
class TestRateDirect:

    def test_should_direct_load_rate(self):
        setup = _rate_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-rate", "live" if setup["live"] else "unit")
        if _skip:
...
def _rate_direct_setup(mockres):
