...
require_relative 'CurrencyExchange_types'
...
class CurrencyExchangeSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = CurrencyExchangeHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = CurrencyExchangeHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, CurrencyExchangeFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    CurrencyExchangeUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = CurrencyExchangeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = CurrencyExchangeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = CurrencyExchangeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue CurrencyExchangeError => err
    ...
    if fetched.is_a?(Hash)
      status = CurrencyExchangeHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = CurrencyExchangeError.new(
        "graphql_error", "CurrencyExchangeSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = CurrencyExchangeSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
