# CurrencyExchange Ruby SDK

The Ruby SDK for the CurrencyExchange API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Convert(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.CurrencyExchange(nil).load(
  { "id" => "test01" }, nil
)
...
```
rb/
├── CurrencyExchange_sdk.rb       -- Main SDK module
