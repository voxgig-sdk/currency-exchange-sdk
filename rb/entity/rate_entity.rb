...
class RateEntity
  ...
  def make
    opts = @_entopts.dup
    RateEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CurrencyExchangeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Rate, Hash] the current Rate data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CurrencyExchangeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Rate fields)
  def match_get
  ...
  # Load a single Rate.
  #
  # @param reqmatch [RateLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Rate.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Rate, Hash] the loaded Rate; raises CurrencyExchangeError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = CurrencyExchangeHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
