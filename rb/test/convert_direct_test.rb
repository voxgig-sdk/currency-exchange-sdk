...
class ConvertDirectTest < Minitest::Test
  def test_direct_load_convert
    setup = convert_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-convert", setup[:live] ? "live" : "unit")
    if _should_skip
...
def convert_direct_setup(mockres)
