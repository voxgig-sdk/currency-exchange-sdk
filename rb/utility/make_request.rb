# CurrencyExchange SDK utility: make_request
...
module CurrencyExchangeUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CurrencyExchangeResponse.new({})
    ...
    if fetch_err
      ...
      response = CurrencyExchangeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CurrencyExchangeResponse.new(fetched)
    else
