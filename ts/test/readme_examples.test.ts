...
//     (\`new CurrencyExchangeSDK(...)\` / \`CurrencyExchangeSDK.test(...)\` become
//     \`CurrencyExchangeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'CurrencyExchangeSDK'
...
const TEST_SEED = {"entity":{"convert":{"test01":{"id":"test01"}},"rate":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CurrencyExchangeSDK(...)\` and any
// \`CurrencyExchangeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CurrencyExchangeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CurrencyExchangeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
